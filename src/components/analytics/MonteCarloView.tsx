      ...
      <div className="mc-layout" style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: '4px' }}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{formatCurrency(balance, currency)}</span>
              </div>
              <input 
                type="range" 
                min={1000} 
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: '4px' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Avg Win per Trade (%):</span>
                <span style={{ fontWeight: 700, color: 'var(--profit-green-light)', fontFamily: 'var(--font-mono)' }}>+{avgWinPct}%</span>
              </div>
              <input 
                type="range" 
                min={0.5} 
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: '4px' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Avg Loss per Trade (Risk %):</span>
                <span style={{ fontWeight: 700, color: 'var(--loss-red)', fontFamily: 'var(--font-mono)' }}>-{avgLossPct}%</span>
              </div>
              <input 
                type="range" 
                min={0.2} 
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: '4px' }}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--theme-secondary)', fontFamily: 'var(--font-mono)' }}>{tradesCount} Trades</span>
              </div>
              <input 
                type="range" 
                min={25} 
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', marginBottom: '4px' }}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--accent-amber)', fontFamily: 'var(--font-mono)' }}>+{targetProfitPct}%</span>
              </div>
              <input 
                type="range" 
                min={5} 
        ...
        <div className="mc-results" style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: 0 }}>
          ...
          <div className="mc-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' }}>
            ...
            <div className="card" style={{ padding: '14px', background: 'var(--bg-main)' }}>
              <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', textTransform: 'uppercase', fontWeight: 600 }}>Median Projected</div>
              <div style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--theme-secondary)', marginTop: '3px' }}>
            ...
            <div className="card" style={{ padding: '14px', background: 'var(--bg-main)' }}>
              ...
              <div style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--accent-amber)', marginTop: '3px' }}>
          ...
          <div className="card mc-chart-card" style={{ padding: '20px' }}>
            <div className="mc-chart-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
              ...
              <div className="mc-legend" style={{ display: 'flex', gap: '12px', fontSize: '0.72rem', flexWrap: 'wrap', rowGap: '4px' }}>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--theme-secondary)' }}>
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#38bdf8' }} />
                  Median (P50)
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--loss-red)' }}>
              ...
              >
                <defs>
                  <linearGradient id="monteCarloAreaGrad" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <stop offset="50%" stopColor="var(--theme-secondary-strong)" stopOpacity="0.1" />
                    <stop offset="100%" stopColor="#ef4444" stopOpacity="0.18" />
                  </linearGradient>
                </defs>

                {/* Grid Lines */}
                <line x1="0" y1={innerHeight * 0.25} x2={chartWidth} y2={innerHeight * 0.25} stroke="var(--border-subtle)" strokeDasharray="3 3" />
                <line x1="0" y1={innerHeight * 0.5} x2={chartWidth} y2={innerHeight * 0.5} stroke="var(--border-subtle)" strokeDasharray="3 3" />
                <line x1="0" y1={innerHeight * 0.75} x2={chartWidth} y2={innerHeight * 0.75} stroke="var(--border-subtle)" strokeDasharray="3 3" />
                ...
                {simulationResult.samplePaths.map((sp) => {
                  ...
                  return (
                    <path
                      ...
                      d={d}
                      fill="none"
                      stroke={sp.finalBalance >= balance ? 'color-mix(in srgb, var(--theme-secondary-strong) 25%, transparent)' : 'rgba(239, 68, 68, 0.2)'}
                      strokeWidth="1"
                    />
                  );
                })}
                ...
                {/* X Axis Labels */}
                <text x="0" y={chartHeight - 8} fill="var(--text-muted)" fontSize="10">Trade #0</text>
                <text x={chartWidth * 0.5} y={chartHeight - 8} fill="var(--text-muted)" fontSize="10" textAnchor="middle">Trade #{Math.floor(tradesCount / 2)}</text>
                <text x={chartWidth} y={chartHeight - 8} fill="var(--text-muted)" fontSize="10" textAnchor="end">Trade #{tradesCount}</text>
              </svg>
            </div>
          </div>
        </div>
